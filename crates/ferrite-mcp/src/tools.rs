...
async fn log_activity(
    ...
    let (status, error_message, result_summary, row_count) = match result {
        Ok(text) => {
            let summary = if text.len() > 200 { &text[..200] } else { text.as_str() };
...
pub async fn list_connections(state: &McpState) -> Result<String> {
    ...
    log_activity(state, "list_connections", None, "List all connections", None, &result, start.elapsed().as_millis() as u64).await;
    result
}
...
pub async fn list_tables(state: &McpState, connection_id: &str, schema: &str) -> Result<String> {
    ...
    log_activity(state, "list_tables", Some(connection_id), &format!("List tables in {schema}"), Some(&params), &result, start.elapsed().as_millis() as u64).await;
    result
}

pub async fn list_columns(state: &McpState, connection_id: &str, table: &str, schema: &str) -> Result<String> {
    let start = Instant::now();
    let params = serde_json::json!({"connection_id": connection_id, "table": table, "schema": schema}).to_string();
    let result = async {
        let uuid = state.require_connection(connection_id).await?;
        let pool_mgr = state.pool_manager.read().await;
        let driver = pool_mgr.get(&uuid).ok_or_else(|| anyhow::anyhow!("Connection lost"))?;
    ...
    log_activity(state, "list_columns", Some(connection_id), &format!("List columns of {schema}.{table}"), Some(&params), &result, start.elapsed().as_millis() as u64).await;
    result
}

pub async fn execute_readonly_query(state: &McpState, connection_id: &str, sql: &str, limit: usize) -> Result<String> {
    let start = Instant::now();
    let params = serde_json::json!({"connection_id": connection_id, "sql": sql, "limit": limit}).to_string();
    let result = async {
        validate_readonly_sql(sql).map_err(|e| anyhow::anyhow!("Read-only validation failed: {e}"))?;
        let uuid = state.require_connection(connection_id).await?;
        let pool_mgr = state.pool_manager.read().await;
        let driver = pool_mgr.get(&uuid).ok_or_else(|| anyhow::anyhow!("Connection lost"))?;
        let qr = driver.execute(sql, &HashMap::new(), limit.min(1000), 0, 30).await.map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    log_activity(state, "execute_readonly_query", Some(connection_id), sql, Some(&params), &result, start.elapsed().as_millis() as u64).await;
    result
}
...
pub async fn explain_query(state: &McpState, connection_id: &str, sql: &str) -> Result<String> {
    ...
    let result = async {
        ...
        let pool_mgr = state.pool_manager.read().await;
        let driver = pool_mgr.get(&uuid).ok_or_else(|| anyhow::anyhow!("Connection lost"))?;
        let er = driver.explain(sql, &HashMap::new()).await.map_err(|e| anyhow::anyhow!("{e}"))?;
        Ok(serde_json::to_string_pretty(&er)?)
    }.await;
    log_activity(state, "explain_query", Some(connection_id), sql, Some(&params), &result, start.elapsed().as_millis() as u64).await;
    result
}
